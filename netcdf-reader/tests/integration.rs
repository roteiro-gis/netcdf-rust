//! Integration tests for netcdf-reader against fixture files.
//!
//! Most fixtures can be generated by running:
//!   scripts/generate-fixtures.sh
//!
//! Generated fixtures are skipped at runtime if missing. Committed compatibility
//! fixtures under testdata/pnetcdf and testdata/parallel are required.
...
fn required_fixture(subdir: &str, name: &str) -> std::path::PathBuf {
    fixture_path(subdir, name).unwrap_or_else(|| {
        panic!(
            "required committed fixture {}/{} is missing from testdata",
            subdir, name
        )
    })
}
...
fn pnetcdf_cdf1_fixed_fixture_reads_as_classic_file() {
    let path = required_fixture("pnetcdf", "pnetcdf_cdf1_fixed.nc");
    let file = netcdf_reader::NcFile::open(&path).unwrap();
...
fn pnetcdf_cdf2_interleaved_record_fixture_reads_record_variables() {
    let path = required_fixture("pnetcdf", "pnetcdf_cdf2_interleaved_records.nc");
    let file = netcdf_reader::NcFile::open(&path).unwrap();
...
fn pnetcdf_cdf5_unsigned_and_64_bit_fixture_reads_new_types() {
    let path = required_fixture("pnetcdf", "pnetcdf_cdf5_unsigned_int64.nc");
    let file = netcdf_reader::NcFile::open(&path).unwrap();
